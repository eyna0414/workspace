...
// UsernamePasswordAuthenticationFilter 는 스프링 시큐리티 로그인 로직의 시작점
// UsernamePasswordAuthenticationFilter 클래스를 상속받아 새로운 클래스를 만든다는 것은
// UsernamePasswordAuthenticationFilter 의 기능을 우리가 변경하여 사용하겠다는 의미.
@Slf4j
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
  ...
  public LoginFilter(AuthenticationManager authenticationManager, JwtUtil jwtUtil){
    ...
    setPasswordParameter("memPw"); // 변경하지 않으면 비밀번호는 password 로 전달
  }

  // attemptAuthentication() 메서드는 로그인 요청이 발생하면 가장 처음 호출됨
