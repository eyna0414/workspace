...
-- 1. 국어점수가 60점 이상이고 영어점수 70점 이상인 학생들의 학번, 이름, 국어점수, 영어점수를
--    조회하되, 학번은 '학생번호', 이름은 '학생명'이라는 별칭을 사용하여 조회하시오.

SELECT STU_NUM 학생번호, STU_NAME 학생명, KOR_SCORE, ENG_SCORE
FROM study_student
WHERE KOR_SCORE >= 60 AND ENG_SCORE >= 70; 

-- 2. 디자인반 혹은 영상반에 소속된 학생 중, 
--    취미가 null이 아닌 학생들의 모든 컬럼 정보를 조회하시오.
...
-- 3. 김씨 성을 지닌 학생 중, 국어점수가 70점에서 85점 사이인 
--    학생들의 학번, 이름, 국어점수를 조회하되, 학번 기준 내림차순으로 정렬하여 조회하시오.
