...
# 단일행 함수의 예시 : 모든 사원들 중 급여가 가장 높은 사원이 급여를 조회
# -> 이 예시의 경우에는 사원수가 달라져도 조회 결과는 무조건 1행만 조회
...
# 결론 1 : 만약, 테이블에 저장된 데이터의 갯수를 알고 싶다면 NULL데이터가 들어간
#				컬럼으로 COUNT() 함수를 사용하는 것은 금지!
# 			-> CONTN 함수는 PK 컬럼으로 진행
...
# 사원들 중 가장 높은 급여와 그 급여를 받는 사원명을 조회
...
# 직급 별 인원 수 조회, 직급기준 내림차순
SELECT COUNT(EMPNO) AS 사원수, JOB
FROM emp
GROUP BY JOB
ORDER BY JOB DESC;
...
# 직급 별 최소 급여를 조회하되, 사원 직급은 제외하며
# 직급별 최소 급여가 300 미만인 데이터도 제외한다.
# 조회 결과 데이터를 직급 기준 오름차순으로 정렬
SELECT JOB, MIN(SAL)
FROM emp
WHERE JOB != '사원' 
GROUP BY JOB
