...
SELECT EMPNO, ENAME, JOB, HIREDATA FROM emp 
WHERE JOB = '대리';

-- 동시에 만족: AND, 둘 중 하나 만족 : OR
-- WHERE 조건1 AND 조건2
-- 급여가 400이상이고, 직급이 '과장'인 사원들의 모든 컬럼을 조회
SELECT * FROM emp 
WHERE SAL >= 400 AND JOB = '과장';

-- EMP 테이블의 모든 사원들 중에 급여가 400이하이거나 700이상인
-- 사원들의 사번, 사원명, 급여를 조회
SELECT EMPNO, ENAME, SAL FROM emp 
...
SELECT EMPNO, ENAME NAME, JOB FROM emp 
WHERE EMPNO >= 1005; 
